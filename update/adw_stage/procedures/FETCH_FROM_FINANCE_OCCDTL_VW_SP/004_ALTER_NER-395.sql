 /***********************************************************************************************************************************************
* Filename:  004_ALTER_NER-395.sql
* Owner: Nectarine
...
CREATE or REPLACE PROCEDURE "ADW_PRODUCT_TRAN"."FETCH_FROM_FINANCE_OCCDTL_VW_SP"()
	...
	try{
		...
		when not matched then insert (COUPON_NK1,COUPON_NK2,COUPON_NK3,BATCH_ID,''stage_finance_occdtl_vw_to_rdv_coupon_hub'',LOAD_TS,SOURCE_SYSTEM_CD)
		values (BARCODE,''~~'',''~~'',BATCH_ID,JOB_NAME,current_timestamp(),''OCCDTL'')`;
