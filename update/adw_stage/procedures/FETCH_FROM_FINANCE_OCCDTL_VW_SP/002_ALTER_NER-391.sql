 /***********************************************************************************************************************************************
* Filename:  002_ALTER_NER-391.sql
* Owner: Nectarine
* Author: Srujan K Alikanti
* Date: 05/03/2022
* Ticket Number: NER-391
...
CREATE or REPLACE PROCEDURE "ADW_PRODUCT_TRAN"."FETCH_FROM_FINANCE_OCCDTL_VW_SP"()
	...
	try{
		...
		ON (T1.COUPON_NK1=T2.BARCODE and T1.HASH_DIFF=T2.HASH_DIFF)
			...
			values
			(
                COALESCE(T2.BARCODE,0),
                ''~~'',
                ''~~'',
                COALESCE(T2.COUPON_NAME ,''~~''),
                COALESCE(T2.COUPON_TAG ,''~~''),
                COALESCE(T2.PRINTER_MESSAGE ,''~~''),
                COALESCE(T2.SHORT_DESC ,''~~''),
                COALESCE(T2.LONG_DESC ,''~~''),
                COALESCE(T2.OFFER_EFFECTIVE_DATE , CURRENT_TIMESTAMP),
                COALESCE(T2.OFFER_EXPIRATION_DATE , CURRENT_TIMESTAMP),
                COALESCE(T2.REWARD_VALUE_FIXED ,0),
                COALESCE(T2.COUPON_PRINT_START_DATE , CURRENT_TIMESTAMP),
                COALESCE(T2.ACCOUNT_CODE ,''~~''),
                COALESCE(T2.COST_CENTRE ,''~~''),
                COALESCE(T2.EST_DIST_QUANTITY , 0),
                COALESCE(T2.EST_REDEMPTION_RATE ,0),
                COALESCE(T2.THRESHOLD ,0),
                COALESCE(T2.VALIDATION_TYPE ,''~~''),
                COALESCE(T2.THRESHOLD_UNITS,0),
                COALESCE(T2.PROMO_ID,0),
                COALESCE(T2.REWARD_VALUE_POINTMULTI ,0),
                COALESCE(T2.NECTAR_PRODUCT_RANGE ,''~~''),
                COALESCE(T2.REWARD_VALUE_MONEY ,0),
                COALESCE(T2.LOYALTY_CARD_REQUIRED ,''~~''),
                COALESCE(T2.INSTORE ,''~~''),
                COALESCE(T2.GOL ,''~~''),
                COALESCE(T2.ROLLING_DAYS_VALIDITY ,0),
                COALESCE(T2.PHOEBUS_CODE ,0),
                COALESCE(T2.SUPPLIER_NAME ,''~~''),
                COALESCE(T2.OC_ID ,0),
                COALESCE(T2.UNIT ,''~~''),
                COALESCE(T2.OWNER ,''~~''),
                COALESCE(T2.REWARD_TYPE ,''~~''),
                COALESCE(T2.CREATEDATE , CURRENT_TIMESTAMP),
                COALESCE(T2.STATUS ,''~~''),
                COALESCE(T2.HASH_DIFF,''~~''),
                ''OCCDTL'',
                PARSE_JSON
                (
                    ''{
