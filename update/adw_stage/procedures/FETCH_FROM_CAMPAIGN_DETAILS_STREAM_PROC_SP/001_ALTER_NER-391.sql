 /***********************************************************************************************************************************************
* Filename:  003_ALTER_NER-391.sql
* Owner: Nectarine
...
CREATE or REPLACE PROCEDURE "ADW_STAGE"."FETCH_FROM_CAMPAIGN_DETAILS_STREAM_PROC_SP"()
    ...
    if(v_rowCnt != 0)
    {
        ...
        snowflake.createStatement( { sqlText: `call "ADW_STAGE"."ADD_LOG_SP"(:1,:2,:3)`, binds:[`INSERT RECORDS`,`INSERT S3 DATA INTO TABLE:"ADW_STAGE"."EAGLEEYE_OCC_CAMPAIGN_DETAILS_BATCH_LOAD", Total Records ingested : `+ v_rowCnt , `Success`] } ).execute() ;
    }
    else
    {
        //no new record is avilable for ingestation
        snowflake.createStatement( { sqlText: `call "ADW_STAGE"."ADD_LOG_SP"(:1,:2,:3)`, binds:[`INSERT RECORDS`,`No new S3 DATA available for ingesation into TABLE:"ADW_STAGE"."EAGLEEYE_OCC_CAMPAIGN_DETAILS_BATCH_LOAD", Total Records ingested : `+ v_rowCnt, `Success`] } ).execute() ;
    }
    }//try block ends
    catch(err)
    {
