 /***********************************************************************************************************************************************
* Filename:  005_ALTER_NER-395.sql
* Owner: Nectarine
* Author: Srujan K Alikanti
* Date: 09/03/2022
* Ticket Number: NER-395
...
CREATE or REPLACE PROCEDURE "ADW_PRODUCT_TRAN"."FETCH_FROM_FINANCE_OCCDTL_VW_SP"()
	...
	try{
        ...
        using (SELECT DISTINCT BARCODE,BATCH_ID FROM "ADW_PRODUCT_TRAN"."FINANCE_OCCDTL_VW") T2
        ON T1.COUPON_NK1=T2.BARCODE
        when not matched then insert (COUPON_NK1,COUPON_NK2,COUPON_NK3,BATCH_ID,JOB_NAME,LOAD_TS,SOURCE_SYSTEM_CD)
        values (BARCODE,''~~'',''~~'',BATCH_ID,''stage_finance_occdtl_vw_to_rdv_coupon_hub'',current_timestamp(),''OCCDTL'')`;
		...
		ON (T1.COUPON_NK1=T2.BARCODE and T1.HASH_DIFF=T2.HASH_DIFF)
			...
			(
                ...
                (
                    ...
                    "JOB_NAME": "stage_finance_occdtl_vw_to_rdv_coupon_occdtl_sat",
